...
$app->post('/workers/create', function (Request $request, Response $response, $args) {
	...
	$action = Admin::addWorker($request_data['first_name'], $request_data['last_name']);
...
$app->get('/workers/get', function (Request $request, Response $response, $args) {
	...
	$response_data['data'] = (new Admin())->getAllWorkers();
	$response->getBody()->write(json_encode($response_data));
	return $response
		->withHeader('Content-type', 'application/json')
		->withStatus(200);
...
// Create Worker Shift //
$app->post('/work-shifts/create', function (Request $request, Response $response, $args) {
	$request_data = $request->getParsedBody();
	$errorMsg = "";
	...
	$worker = new \Michaelthedev\Workershifts\Worker($request_data['worker_id']);
	if (!$worker->exists()) {
	...
	$action = $worker->addWorkShift($request_data['start_date'], $request_data['start_time']);
	if ($action['status'] == 'success') {
		$response_data['error'] = false;
		$response_data['message'] = $action['message'];
		$response_data['data']['worker_id'] = $action['worker_id'];
...
// Get Work shifts //
$app->get('/work-shifts/get', function (Request $request, Response $response, $args) {
	$response_data['error'] = false;
	$response_data['message'] = 'success';
	$response_data['data'] = (new Admin())->getAllWorkShifts();
	$response->getBody()->write(json_encode($response_data));
	return $response
		->withHeader('Content-type', 'application/json')
		->withStatus(200);
